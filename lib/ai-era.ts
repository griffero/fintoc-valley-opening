...
export function createSoraStudio(
  ...
  const studio = group(
    world,
    'Sora · studio shutdown · April 2026',
    SORA_SITE.x,
    0,
    SORA_SITE.z,
  );
  ...
  box(studio, 0, 5.6, -1.5, 19.1, 0.35, 9.7, mat('#ded9ca', 'roof'));
  const sign = group(studio, 'Sora rooftop sign · folds down', 0, 6.15, 1.3);
  box(sign, 0, 0, 0, 14.5, 3.8, 0.4, mat('#344c57', 'paint'));
  text('SORA', 2.6, 0.2, '#e7efec', sign, 0, 0.45, 0.25);
  const clapper = group(sign, 'Studio clapperboard', 0, 3.8, 0);
  box(clapper, 0, 0, 0, 14.5, 0.55, 0.43, mat('#eff1e9', 'paint'));
  for (let i = 0; i < 8; i++) {
    const stripe = box(
      clapper,
      -6.7 + i * 1.9,
  ...
  batch(clapper);
  batch(sign);
  const screen = group(studio, 'Sora studio screen', 0, 1.3, 3.18);
  box(screen, 0, 0, 0, 10.4, 2.7, 0.06, mat('#86b8bb', 'paint'));
  const filmFrame = text('PLAY', 1.15, 0.06, '#e5f0dc', screen, 0, 0.65, 0.05);
  const shutter = group(studio, 'Sora · closing roller shutter', 0, 4.95, 3.33);
  for (let row = 0; row < 15; row++)
    box(
      ...
      0.25,
      0.12,
      mat(row % 2 ? '#7a8580' : '#89938b', 'metal'),
    );
  text('CLOSED', 1.5, 0.08, '#eee9d8', shutter, 0, -2.3, 0.1);
  batch(shutter);
  shutter.userData.stepScale = true;
  const placard = group(studio, 'Sora · closure date placard', 7.1, 0.3, 5.8);
  box(placard, 0, 0.3, 0, 4.8, 2.45, 0.2, mat('#e8dcca', 'paint'));
  text('APR 2026', 0.54, 0.045, '#4c4b43', placard, 0, 1.6, 0.13);
  text('SORA', 0.7, 0.06, '#4c4b43', placard, 0, 0.6, 0.13);
  batch(placard);
  for (let i = 0; i < 3; i++) {
    const crate = group(
      studio,
      `Sora · packed equipment ${i + 1}`,
      -6 + i * 2.4,
      0.3,
      5.6,
    );
    box(crate, 0, 0, 0, 1.65, 1.15, 1.2, mat('#a99372', 'wood'));
    for (const x of [-0.45, 0.45])
      box(crate, x, 0, 0.61, 0.15, 1.17, 0.04, mat('#6c6659', 'paint'));
    batch(crate);
    motion.track(crate, (t) => {
      crate.scale.setScalar(visibleScale(t >= 3.92 + i * 0.08));
    });
  }
  motion.track(sign, (t) => {
    sign.rotation.x = -ease(3.88, 4.4, t) * Math.PI * 0.46;
    sign.position.y = 6.15 - ease(3.88, 4.4, t) * 0.25;
  });
  motion.track(shutter, (t) => {
    const closing = ease(3.55, 4.08, t);
    shutter.position.y = 4.95 + (1 - closing) * 4.5;
    shutter.scale.setScalar(visibleScale(t >= 3.55));
  });
  motion.track(screen, (t) => {
    screen.scale.setScalar(visibleScale(t < 3.8));
  });
  motion.track(placard, (t) => {
    placard.scale.setScalar(visibleScale(t >= 3.95));
  });
  // The screen uses rigid geometry; the closure remains readable in the GLB.
  filmFrame.name = 'Sora screen · PLAY';
  const camera = group(studio, 'Movie camera on tripod', -6.7, 0.3, 5.2);
  box(camera, 0, 1.4, 0, 0.75, 0.65, 0.5, mat('#303c3b', 'metal'));
  mesh(
    camera,
    new THREE.CylinderGeometry(0.18, 0.21, 0.5, 12),
    mat('#182b30', 'glass'),
    0.48,
    1.7,
    0,
  ...
  for (let i = 0; i < 3; i++) {
    const leg = box(
      camera,
      Math.cos(i * 2.094) * 0.22,
      0,
      Math.sin(i * 2.094) * 0.22,
      0.07,
      1.5,
      0.07,
      mat('#5a6661', 'metal'),
    );
    leg.rotation.z = Math.cos(i * 2.094) * 0.25;
    leg.rotation.x = Math.sin(i * 2.094) * 0.25;
  }
  batch(camera);
  batch(studio);
  return SORA_SITE;
