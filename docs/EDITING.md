...
- **Exportar modelo 3D** downloads a GLB containing editable scene geometry, materials, instanced vegetation/vehicles, lights and a named animation clip. It includes the individual old/new Fintoc logo pieces, the unbranded expanding office, rigid title floors, beveled roofs, foundations and steel frames, the reference-based headquarters and its animated rooftop guests, balloon inflation/flight, crane slewing/trolleys/hoists, excavator joints, mixer truck and facade identity replacement. The browser camera, postprocessing and the soundtrack remain in the web project; they are not baked into the GLB. Regular vehicle traffic uses procedural instance matrices in the web renderer.
...
- `lib/fintoc-identity.ts`: one white sign with the old-to-new Fintoc logo transformation in black.
...
Fintoc appears only on the final headquarters. Both authentic identities share one white rooftop panel, with fixed black extruded lettering. The 2021 outline assembles from 5.15–5.80 seconds, remains legible, and disassembles from 6.35–6.83. The current identity then assembles larger from 6.88–7.60, centered in the same place before the final camera hold. The roadside Fintoc billboard and the balloon emblem have been removed. The first office keeps its construction and rooftop activity as the selectable, unbranded **Oficina en expansión**.

`lib/fintoc-identity.ts` contains the common sign, black lettering material and individual SVG component transforms. Both logo groups stay present in the GLB with scale-based animation. **Fondo Fintoc** adjusts the shared panel color; project versions 1/2 migrate their previous default dark Fintoc colors to white once, while custom palette values are retained. Version 3 preserves all newly chosen panel colors.
