...
- `lib/fintoc-identity.ts`: one black sign with the old-to-new Fintoc logo transformation in white.
...
Fintoc appears only on the final headquarters. Both authentic identities share one black rooftop panel, with fixed white extruded lettering. The 2021 outline assembles from 5.15–5.80 seconds, remains legible, and disassembles from 6.35–6.83. The current identity then assembles larger from 6.88–7.60, centered in the same place before the final camera hold. The roadside Fintoc billboard and the balloon emblem have been removed. The first office keeps its construction and rooftop activity as the selectable, unbranded **Oficina en expansión**.

`lib/fintoc-identity.ts` contains the common sign, white material and individual SVG component transforms. Both logo groups stay present in the GLB with scale-based animation. **Fondo Fintoc** adjusts the shared panel color; the old default dark letter color migrates to black while custom palette values are retained.
